...
from quantile_network_pytorch import QuantileNet, EarlyStopping, make_dataset
...
def load_data(config, logger=None):
    dfs = []
    if logger != None:
        logger.debug(f"Looking for files matching: {config['data']['location']}")

    file_list = sorted(glob.glob(config["data"]["location"]))

    if not file_list and logger != None:
        logger.error(f"No files found for pattern: {config['data']['location']}")
        raise FileNotFoundError(f'No files found for pattern: {config["data"]["location"]}')

    for filepath in file_list:
        if logger != None:
            logger.info(f"Loading {filepath}")
        df = pd.read_json(filepath).set_index('ievt_n')
        if df.isnull().values.any() and logger != None:
...
def load_and_prepare_data(config, logger):
    ...
    logger.info(f"Data loaded with shapes - X: {xdata.shape}, Y: {ydata.shape}")
    ...
    logger.debug(f"Train indices: {train_idx[:5]}... Test indices: {test_idx[:5]}...")

    # Save indices for reproducibility
    np.save(config['output']['train_idx'], train_idx)
    np.save(config['output']['test_idx'], test_idx)
    logger.info(f"Train/test indexes saved: {config['output']['train_idx']}, {config['output']['test_idx']}")
    ...
    logger.info(f"Train/test split done. Train X shape: {train_in.shape}, Test X shape: {test_in.shape}")
...
def normalise_data(train_in, test_in, train_out, test_out, logger, config):
    ...
    for i in range(input_dims):
        ...
        if sigma == 0:
            logger.warning(f"Input feature {i} has zero std — may indicate constant feature.")
    ...
    norm_dict = {
        "input": [{"mean": float(mu), "std": float(sigma)} for mu, sigma in norm_info_in],
        "output": [{"mean": float(mu), "std": float(sigma)} for mu, sigma in norm_info_out]
    }
    ...
    return train_in_norm, test_in_norm, train_out_norm, test_out_norm, norm_info_in, norm_info_out
...
def prepare_iqn_datasets(train_in_norm, train_out_norm, config, logger):
    ...
    x_val, y_val = make_dataset(train_in_norm, train_out_norm, input_dims, output_dims, n_samples)
    logger.info(f"make_dataset returned shapes: x_val {x_val.shape}, y_val {y_val.shape}")

    iqn_train_in, iqn_val_in, iqn_train_out, iqn_val_out = train_test_split(
        x_val, y_val, test_size=config['training']['val_size'], random_state=config['random_seed']
    )
...
def build_model(config, input_dims):
    ...
    # Add layers
    model.add(nn.Linear(in_features=input_dims + 1, out_features=config['structure']['width']))
    model.add(nn.LeakyReLU())

    for _ in range(config['structure']['hidden'] - 1):
        model.add(nn.Linear(in_features=config['structure']['width'], out_features=config['structure']['width']))
        model.add(nn.LeakyReLU())

    model.add(nn.Linear(in_features=config['structure']['width'], out_features=config['structure']['layer_output_dims']))

    return model

def train_loop(model, train_loader, val_loader, config, logger):
    optimizer = optim.Adam(model.parameters(), lr=config['training']['initiallr'], amsgrad=False)
    ...
    for cycle in range(config['training']['cycles']):
        ...
        for epoch in range(config['training']['epochs']):
            ...
            logger.debug(f"Epoch {epoch+1}: mean train loss = {mean_train_loss:.4f}, mean val loss = {mean_val_loss:.4f}")
...
def main():
    ...
    train_in, test_in, train_out, test_out = load_and_prepare_data(config, logger)
    if args.plotting:
        for label, data in zip(['in', 'out'], [train_in, train_out]):
            for i in range(data.shape[1]):
                plt.figure()
                plt.hist(data[:, i], bins=300)
                plt.xlabel(f"input feature {i}")
                plt.ylabel("counts")
                plt.savefig(f"plots/train_{label}_feature_{i}.png")

    train_in_norm, test_in_norm, train_out_norm, test_out_norm, norm_info_in, norm_info_out = normalise_data(train_in, test_in, train_out, test_out, logger, config)
    if args.plotting:
        for label, data in zip(['in', 'out'], [train_in_norm, train_out_norm]):
            for i in range(data.shape[1]):
                ...
                plt.xlabel(f"input feature {i}")
                plt.ylabel("counts")
                plt.savefig(f"plots/train_norm_{label}_feature_{i}.png")

    iqn_train_in, iqn_val_in, iqn_train_out, iqn_val_out = prepare_iqn_datasets(train_in_norm, train_out_norm, config, logger)

    if args.plotting:
        for one_hot_encode in [5, 6]:
            mask = np.where(iqn_train_in[:, one_hot_encode] == 1)[0]
            fig = plt.figure()
            plt.hist(iqn_train_in[mask, 2], bins=300)
            plt.xlabel("Input Feature 2")
            plt.ylabel("Counts")
            plt.savefig(f"plots/iqn_train_input_feature_2_ohe_{one_hot_encode}.png")

            fig = plt.figure()
            plt.hist(iqn_train_out[mask], bins=300)
            plt.xlabel("Output Feature 1")
            plt.ylabel("Counts")
            plt.savefig(f"plots/iqn_train_output_feature_ohe_{one_hot_encode}.png")

            fig = plt.figure()
            plt.scatter(iqn_train_in[mask, 2], iqn_train_out[mask], alpha=0.2)
            plt.xlabel("Input Feature 2")
            plt.ylabel("Output Feature")
            plt.savefig(f"plots/iqn_train_input_vs_output_ohe_{one_hot_encode}.png")
    ...
    train_loader = DataLoader(train_dataset, batch_size=config['training']['batch_size'], shuffle=False, drop_last=True)
    ...
    for epoch, tr_loss, vl_loss in zip(epoch_list, train_loss, val_loss):
        logger.info("Epoch %3d | Train Loss: %.6f | Val Loss: %.6f", epoch, tr_loss, vl_loss)
