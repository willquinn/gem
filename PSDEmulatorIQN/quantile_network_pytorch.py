...
class QuantileNet(nn.Module):
    def __init__(self, grad_loss_scale=100, tanh_loss_scale=100, network_type="normalizing", clip=1e-7):
    ...
    def normalizing_call(self, inputs):
        ...
        grad_loss = torch.tensor(0.0, dtype=torch.float32, device=inputs.device)

        # Sample quantiles for median +/- 1 std
        quantiles_low = (-2.04 * torch.ones(1, count, device=inputs.device)).pow(1)
        quantiles_mid = torch.zeros(1, count, device=inputs.device)
        quantiles_high = (2.04 * torch.ones(1, count, device=inputs.device)).pow(1)
        ...
        grads_norm = torch.autograd.grad(out_norm.sum(), inputs_q, create_graph=True)[0][:, -1]
        loss = self.grad_loss_scale * (torch.where(grads_norm < 0, grads_norm, torch.tensor(0.0, device=grads_norm.device))) ** 2
        grad_loss += loss.mean()

        grads_base = torch.autograd.grad(out_base.sum(), inputs_q, create_graph=True)[0][:, -1]
        loss = self.grad_loss_scale * (torch.where(grads_base < 0, grads_base, torch.tensor(0.0, device=grads_base.device))) ** 2
        grad_loss += loss.mean()

        # Transform normalized output
        out_norm = (out_norm + 3) / 6
        abs_out_norm = torch.abs(out_norm)
        loss = self.tanh_loss_scale * torch.where(abs_out_norm > 1, (abs_out_norm - 1) ** 2, torch.tensor(0.0, device=abs_out_norm.device))
        grad_loss += loss.mean()

        output = torch.cat([out_norm, out_base, scale, shift, quantiles.T], dim=1)
        return grad_loss, output

    def no_normalizing_call(self, inputs, training=True):
        count = inputs.shape[0]  # batch size
        quantiles = torch.rand(count, 1, device=inputs.device) * 6 - 3  # [B, 1]
        ...
        if training:
            out_base = val
            grads = torch.autograd.grad(out_base.sum(), inputs_q, create_graph=True)[0][:, -1]
            loss = self.grad_loss_scale * (torch.where(grads < 0, grads, torch.tensor(0.0, device=grads.device))) ** 2
            grad_loss += loss.mean()
        else:
...
def sample_net(
    quantile_object, quantile_samples, inputs, input_count,
    input_dims, output_dims, network_type="normalizing", clip=1e-7
):
    clip = 1 - clip
    ...
    predict_inner = predict_normalizing if network_type == "normalizing" else predict_no_normalizing
    ...
    random_quant = torch.rand((1, quantile_samples * input_count), device=device) * 6 - 3
    extended_input = inputs.repeat_interleave(quantile_samples, dim=0).T
    zero_inputs = torch.zeros((output_dims - 1, quantile_samples * input_count), device=device)

    final_inputs = torch.cat([extended_input, sampling_locs, zero_inputs, random_quant], dim=0).T
    ...
    def predict_loop(i, val, output):
        ...
        for j in range(output_dims):
            one_hot = torch.ones_like(val[:, 0:1]) if j == location else torch.zeros_like(val[:, 0:1])
            new_input.append(one_hot)

        # Previous outputs
        start = input_dims + output_dims
        mid = start + old_location
        end = start + output_dims - 1
    ...
    def predict_main(val):
        ...
        final_output = torch.cat([val[:, start:mid], output, val[:, mid + 1:-1]], dim=1)
        return final_output
