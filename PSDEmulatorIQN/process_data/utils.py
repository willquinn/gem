    ...
    with open(pet_filename, "a") as outfile:
        ...
        if isinstance(Es, list):
            for index in range(len(Es)):
                outfile.write(
                    f"{ns[index]}, 1, {ts[index]}, {Es[index]},"
                    + f" {xs[index]}, {ys[index]}, {zs[index]}\n"
                )
        else:
            outfile.write(f"{ns}, 1, {ts}, {Es}, {xs}, {ys}, {zs}\n")


def nested_drift_time(fn, r_array, z_array):
    if isinstance(r_array, ak.Array) and isinstance(z_array, ak.Array):
        return ak.Array(
            [nested_drift_time(fn, r, z) for r, z in zip(r_array, z_array)]
        )
    else:
...
class ReadDTFile:
    ...
    def GetIndex(self, r, z):
        ...
        i = int(
            (rGrid - self.minr) / self.dr * numZ
            + (zGrid - self.minz) / self.dz
        )
