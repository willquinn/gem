...
from utils import GED_NAME, gaus, linear

usage = ""

parser = argparse.ArgumentParser(description=usage)
parser.add_argument(
    "-i", "--input", type=str, required=True, help="Input PSS file"
)
parser.add_argument(
    "-o",
    "--output",
    default=f"data/{GED_NAME}_pss_output_default.json",
    type=str,
...
parser.add_argument(
    "-c",
    "--dsp_config",
    default="dsp_proc_chain_emmulator.json",
    type=str,
...
parser.add_argument(
    "-dsp",
    "--dsp_output",
    default="data/dsp_output.hdf",
    type=str,
    help="Name of dsp_config file",
)
parser.add_argument(
    "-plot", "--plot", default=False, type=bool, help="Plot outputs"
)
parser.add_argument(
    "-b", "--build", default=False, type=bool, help="Plot outputs"
)
...
def main():
    ...
    freq, bin_edges = np.histogram(
        dsp_data.trapEmax.values, range=(35750, 36250), bins=500
    )
    bin_width = bin_edges[-1] - bin_edges[-2]
    bin_centres = bin_edges[:-1] + bin_width / 2

    popt, pcov = curve_fit(
        f=gaus, xdata=bin_centres, ydata=freq, p0=[36100, 10, 100, 1]
    )
    initial_m = 2614.511 / popt[0]

    peaks = np.array(
        [583.191, 727.330, 860.564, 1592.511, 1620.50, 2103.511, 2614.511]
    )
    ...
    for index, peak in enumerate(peaks):
        ...
        freq, bin_edges = np.histogram(
            dsp_data.query(
                f"(trapEmax > {(peak-15)/initial_m})"
                + f" & (trapEmax < {(peak+15)/initial_m})"
            ).trapEmax.values
            * initial_m,
            range=(peak - lowE, peak + highE),
            bins=40,
        )
        ...
        popt, pcov = curve_fit(
            ...
            ydata=freq,
            p0=[peak, 10, 100, 1],
            bounds=[[peak - lowE, 0, 0, 0], [peak + highE, 20, 10000, 1000]],
        )
        popts.append(popt)
        pcovs.append(pcov)
    ...
    popt, pcov = curve_fit(
        f=linear,
        xdata=peaks,
        ydata=[h[0] for h in popts] / initial_m,
        sigma=np.array([np.sqrt(h[0, 0]) for h in pcovs]) / initial_m,
        p0=[initial_m**2, 0],
    )
    ...
    if args.plot:
        ...
        ax1.errorbar(
            peaks,
            [h[0] for h in popts] / initial_m,
            yerr=[np.sqrt(h[0, 0]) for h in pcovs] / initial_m,
            fmt=".",
            label=r"fitted $\mu$",
        )
        ...
        ax2.errorbar(
            peaks,
            linear(peaks, *popt) - [h[0] for h in popts] / initial_m,
            yerr=[np.sqrt(h[0, 0]) for h in pcovs] / initial_m,
            fmt=".",
        )
        ...
        fig = plt.figure()
        plt.hist(
            dsp_data["AoE"].values, range=(0, 1), bins=2000, histtype="step"
        )
        plt.xlabel("A/E")
    ...
    ys_df = pd.DataFrame(
        {"Es": dsp_data["trapEmax_cal"], "AoEs": dsp_data["AoE"].values}
    )
