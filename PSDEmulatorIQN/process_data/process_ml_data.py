...
def main():
    ...
    new_indexes = pd.read_csv("data/new_index.csv").Column1.values.astype(
        "int"
    )

    # Iterate through each group
    for ievt_n, group in grouped:
        if ievt_n in new_indexes:
            continue
        ...
        for i in range(n_clusters):
            clust_index = (
                sorted_indices[i] if len(sorted_indices) > i else None
            )
