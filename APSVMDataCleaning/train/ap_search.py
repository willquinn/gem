...
argparser.add_argument(
    "--file", help="file containing training data", type=str, required=True
)
argparser.add_argument(
    "--exemplars", help="number of clusters desired", type=int, required=True
)
...
print(
    "--- %s minutes elapsed for similarities---"
...
def ap_search(pref, damp, verbose=True):
    ...
    if verbose == True:
        print(
            "--- %s minutes elapsed for AP iteration---"
            % ((time.time() - start_time) / 60)
        )
